...
export function SessionControls() {
  ...
  const speakingEnergy = localMultibandVolume.reduce(
    (s, arr) => s + arr.reduce((a, b) => a + b, 0),
    0,
  );
  const isSpeaking = !isMuted && speakingEnergy > 0.25; // simple heuristic

  return (
    <div className="flex flex-row gap-2">
      <div className="flex items-center rounded-md bg-neutral-100 text-secondary-foreground">
        <div className="flex gap-1 pr-4 items-center">
          <TrackToggle
          ...
          >
            {isMuted ? (
              ...
              <Mic className={`h-4 w-4 ${isSpeaking ? "text-green-600" : ""}`} />
            )}
          </TrackToggle>
          <div className="flex flex-col items-center justify-center">
            <MultibandAudioVisualizer
              state={isMuted ? "idle" : isSpeaking ? "speaking" : "listening"}
              barWidth={2}
              minBarHeight={2}
              maxBarHeight={16}
              ...
              gap={2}
            />
            <span className="text-[10px] mt-0.5 font-medium text-neutral-500">
              {isMuted
                ? "Muted"
                : isSpeaking
                ? "Speaking"
